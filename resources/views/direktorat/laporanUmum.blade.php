<x-direktorat-app-layout>
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Daftar Unit Kegiatan Mahasiswa dan Organisasi Mahasiswa yang ada di Telkom University.</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-x-6 gap-y-8">
        @foreach($ukmOrmawas as $item)
            <div class="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                <a href="{{ route('direktorat.laporan-umum.show', ['slug' => $item->slug]) }}" class="block group">
                ...
                <div class="p-5 flex flex-col flex-grow">
                    ...
                    <h3 class="text-xl font-bold text-gray-800 mb-1 hover:text-red-600 transition-colors">
                            <a href="{{ route('direktorat.laporan-umum.show', ['slug' => $item->slug]) }}">{{ $item->name }}</a>
                    </h3>
                    ...
                    <div class="mt-auto pt-4 border-t border-gray-200 space-y-2">
                        <a href="{{ route('direktorat.laporan-umum.show', ['slug' => $item->slug]) }}" class="flex items-center justify-center w-full px-4 py-2.5 text-sm font-medium text-indigo-700 bg-indigo-100 rounded-md hover:bg-indigo-200 transition-colors">
